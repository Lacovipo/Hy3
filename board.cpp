...
void Board::set_from_fen(const std::string& fen) {
    for (int i = 0; i < N_SQ; i++) squares[i] = make_piece(WHITE, NONE);
    side = WHITE;
    ep_square = NO_SQ;
    castling = 0;
    halfmove = 0;
    ...
    if (ep_part != "-") ep_square = string_to_square(ep_part);
    else ep_square = NO_SQ;
    if (!half_part.empty()) halfmove = std::stoi(half_part);
    if (!full_part.empty()) fullmove = std::stoi(full_part);
...
void Board::make_move(Move m, Undo& u) {
    ...
    if (flag == FLAG_EP) {
        ...
        squares[cap_sq] = make_piece(WHITE, NONE);
    } else {
    ...
    tog(moving, from);
    squares[to] = moving;
    squares[from] = make_piece(WHITE, NONE);
    ...
    if (flag == FLAG_CASTLE) {
        ...
        if (rf != NO_SQ) {
            ...
            tog(rook, rf);
            squares[rt] = rook;
            squares[rf] = make_piece(WHITE, NONE);
            tog(rook, rt);
        }
    }
    ...
    if (pt == PAWN || u.captured != make_piece(WHITE, NONE) || flag == FLAG_EP)
        halfmove = 0;
    else
        halfmove++;
...
void Board::unmake_move(Move m, const Undo& u) {
    ...
    squares[from] = moving;
    squares[to] = make_piece(WHITE, NONE);

    // Restaurar capturada
    if (u.captured != make_piece(WHITE, NONE)) {
        if (flag == FLAG_EP) {
            Square cap_sq = make_sq(sq_file(to), sq_rank(from));
...
// Static Exchange Evaluation (SEE), formulación estándar de intercambio.
// Devuelve el balance neto (en centipeones) del intercambio que comienza con
// el lado 'us' capturando la pieza en 'to', asumiendo que ambos lados juegan
// la captura más barata disponible.
namespace {
    ...
    // mutable 'occ' (excluye la propia casilla 'to'). Devuelve -1 si ninguno.
    Square find_least_attacker(Color by, Square to, Piece occ[64]) {
        int best_sq = -1;
        int best_val = 1 << 30;
        int dir = (by == WHITE) ? 1 : -1;
        for (int df : {-1, 1}) {
...
int Board::see(Square to, Color us) const {
    ...
    // Primera captura: el atacante más barato de 'us' sobre 'to'.
    Square from = find_least_attacker(us, to, occ);
    if (from < 0) return 0;  // 'us' no puede capturar: intercambio vacío.
    #if 0
    std::cerr << "DEBUG see to=" << to << " us=" << us << " from=" << from
              << " occ[to]=" << (int)piece_type(occ[to]) << "\n";
    #endif

    int gain[32];
    int n = 0;
    gain[n++] = see_pv(occ[to]);                 // valor de la víctima inicial
    occ[to] = occ[from];
    occ[from] = make_piece(WHITE, NONE);

    Color side = Color(us ^ 1);
    while (true) {
        from = find_least_attacker(side, to, occ);
        if (from < 0) break;
        gain[n] = see_pv(occ[to]) - gain[n - 1];  // gana víctima, paga lo anterior
        occ[to] = occ[from];
        occ[from] = make_piece(WHITE, NONE);
        n++;
        side = Color(side ^ 1);
    ...
    // Resolución hacia atrás: el que inicia maximiza, el defensor minimiza.
    for (int i = n - 2; i >= 0; i--)
